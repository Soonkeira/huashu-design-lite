...
开工前问几个问题，按 Design Mode 走（见 SKILL.md「设计复杂度路由」）：FULL 走下面这份完整必问清单，至少 10 个问题；STANDARD 最多提出 2 个真正影响设计的问题，清单里的其余项自己按 best judgment 补 assumption（标 assumption），不要一次性甩 10 个问题给用户；FAST 不问，除非存在真正阻塞项。不是走过场，是真的要把需求摸清——但问题密度要跟设计不确定性匹配。
...
FULL 模式必须问清这5类问题；STANDARD 只问其中真正影响设计的 ≤2 个，其余自己补 assumption：
