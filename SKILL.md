...
Workflow complexity must scale with design uncertainty, not task existence.

收到设计任务后，先判断模式。除非用户明确指定 `/design fast`、
`/design standard`、`/design full`，否则自动判断。

#### FAST

适用于：
- 已有页面/设计上的局部修改
- 调整间距、留白、字号、颜色、对齐、组件样式
- 修复 responsive / overflow / clipping
- 已选定设计方向后的继续修改
- 用户提供了明确参考，并要求直接照该方向实现
- 机械性视觉任务
...
- 不写长篇设计 spec
- 没有真正阻塞项时不提澄清问题
- 默认只读取当前任务最相关的 0-1 个 reference
- 涉及真实品牌/产品/数据时仍执行事实与资产验证
- 直接实现
- 完成后至少检查 Desktop + Mobile
...
- App 页面/原型
- 普通信息图
- 已有一定 design context，但仍存在视觉方向选择

行为：
- 使用简版 design spec
- 最多提出 2 个真正影响设计的问题
- 默认生成 2 个明显不同的真实视觉方向
- 如果用户已经给出非常明确的 reference / brand system / approved direction，
  可直接做 1 个主方案，不为了凑数制造无意义变体
- references 按需加载，通常不超过 2 个
- 用户需要方向选择时，展示后等待选择
- 选定后完成实现和验证
...
- 高价值正式官网
- 正式产品发布页
- Launch Film / Campaign
- 正式对外 PPT / Pitch Deck
- 用户明确要求探索多个方向
- 需求高度模糊，设计不确定性高
- 用户明确要求 highest quality / full exploration

行为：
- 保留原 Huashu Design 完整工作流
- 完整事实/品牌资产验证
- 三方向真实视觉
- 完整设计 spec
- 用户选择 Gate
...
- `/design full`
- 「直接做，不用出方向」→ FAST
- 「给我几个方向看看」→ STANDARD 或 FULL
- 「完整探索后再做」→ FULL

不要因为任务属于“设计”就自动升级到 FULL。
...
| 会产出新视觉设计 | 先执行「设计复杂度路由」：FAST 直接执行；STANDARD 走轻量方向探索；FULL 才进入完整三方向硬门 |
...
| launch film/品牌宣传片（「Apple级」「超级碗品质」） | **默认 FULL → 3 方向**（方向板级初稿，见 Fallback「多方向初稿形态」）→ 用户选定后再写万字 director's notes → `references/launch-film-director-notes.md`；服从显式 Design Mode override（`/design fast`、「直接做、不出方向」→ 降为 FAST 照做，不硬门） |
| App/iOS 原型 | 「App / iOS 原型专属守则」（覆盖通用规则） |
| 评审/打分 | Step 10 → `references/critique-guide.md` |
| runtime 能力受限（无法 spawn subagent / 上下文吃紧 / 工具能力有限） | 上述任一条 + 「Runtime 能力降级」 |

例：「做个咖啡主题的 PPT」= 第 2 行 + 第 3 行——普通新设计走 STANDARD，出 2 个明显不同的真实方向（咖啡是主题不是品牌，不找 logo），deck 骨架统一用概览墙模板。
再例：「做个苹果宣传片风格的 30s 动画」——指定风格不等于直接开工，也不等于必须三方向：正式发布级宣传片属 FULL，在 Apple 语境内出 3 个差异化诠释的方向板让用户选（如深空暗场版 / 大白底衬线版 / 产品色沉浸版）；若用户同时说「就按这个方向直接做」，则降为 FAST 照做。
...
FAST：
- 完全跳过本节 Phase 1-5
- 直接沿现有 design context 修改
- 不创建无意义的视觉变体

STANDARD：
- 当视觉方向尚未确定时触发
- 默认生成 2 个明显不同的真实视觉方向
- 如果用户已经提供明确 reference、品牌系统、现有设计语言或 approved direction，
  可以跳过方向探索，直接实现
- 不允许为了满足流程而制造只有配色不同的假变体

FULL：
- 执行完整三方向流程
- 保留本节原有 Phase 1-7 的质量标准

以下情况直接跳过方向探索：
- 用户明确说「直接做」「不用出方案」
- 已选定方向后的迭代
- 局部视觉修改
- bug / responsive / overflow 修复
- 纯文字、导出、截图等机械操作
- 用户提供了足够明确的目标稿并要求忠实实现

原则：
只有「存在值得用户决策的设计分歧」时，才生成多个方向。

**多方向初稿形态（按产出类型定义，必须是看得见的真实视觉，不是文字描述；数量按 Design Mode：FULL 3 / STANDARD 2）**：
...
**展示后必须停**：多方向摆出来后**结束回合等用户选择**，不得自行选定继续执行——包括 autonomous / 无人值守会话（这是真正只有用户能做的决策，停轮不算阻塞）。

### 完整流程（7 个 Phase，顺序执行；Phase 3.5 是图片前置半步）

**Phase 1 · 对话澄清需求 + 主动索要参考（不要跳过、不要直接开做）**
先用**对话**了解（问题数上限按 Design Mode，与路由表一致：FULL 一次最多 3 个 / STANDARD 一次最多 2 个）：目标受众 / 核心信息 / 情感基调 / 输出格式。
...
⏱️ **无应答策略**：问题发出后，若用户**没回应任何信息**（只丢了最初那句模糊需求就没下文）→ 不要枯等。按 best judgment 补齐假设（标 assumption），直接往下跑完 Phase 2-4 把当前 Design Mode 要求的几版真实视觉摆出来——**用「看得见的东西」代替继续追问**（正好呼应选择无效铁律）。

> 用户给了**具体品牌/产品名（能去官网找到 logo 的那种，如 Stripe / DJI / 某 App）**或品牌资产/参考站 → **加走「§1.a 核心资产协议」取齐资产**：各方向全部基于同一套真实品牌资产做，差异在设计诠释（旧规则「品牌名→跳出 Fallback」已废止）。方向数量按 Design Mode 定（FULL 3 个 / STANDARD 2 个）——品牌资产齐不等于方向已定。
> ⚠️ **普通主题名不算品牌名**：「咖啡 / 鹦鹉 / 历史 / 健身」这类是**内容主题**，不是可找 logo 的品牌——不要跑去找「咖啡的 logo」空转。

**Phase 2 · 顾问式重述**（把需求真正嚼透，不是敷衍一句——篇幅按任务复杂度决定，不设机械字数目标）
用自己的话深入重述本质需求、受众、场景、情感基调、用户没说出口的潜在期待。以「基于这个理解，我**直接做几个不同方向的真实版本给你看**」结尾（数量按 Design Mode：FULL 3 个 / STANDARD 2 个）——❌ 不要以「你想选哪个方向？」结尾（见 Phase 3 铁律）。

**Phase 3 · 固化设计 spec**

FAST：
跳过独立 spec 文件，直接从现有项目 design context 工作。

STANDARD：
使用 compact spec，控制在足够明确但不过度展开的程度：
...
每项 1-3 行即可。禁止为了满足长度要求写重复性设计散文。

FULL：
使用完整 design spec。应覆盖产品、受众、场景、信息结构、
情绪气质、输出尺寸、品牌约束、图片需求、视觉母题等。
完整程度优先，不设机械字数目标。

**所有模式共用**：这是各方向 subagent 的唯一共同输入，它们各自独立工作、只看 spec、互不参考。**输出格式与尺寸必填**（网页还是 PPT？具体像素？）——多个方向必须用同一尺寸，否则尺寸不一无法横向对比。

**Phase 3.5 · 🔴 CHECKPOINT 图片素材前置（spawn 多方向 subagent 前必做，硬要求）**
...
**图片必需 → 先制定获取策略、取齐真图，再 spawn 多方向 subagent**（各方向 subagent 共用同一批真图，只换设计），绝不边设计边用色块糊弄：
...
🔴 **具名产品 logo 子门（spawn 多方向 subagent 前必过，硬要求）**：把设计里会出现的产品 / 品牌名**逐个列成清单**，确认每个都已取到官方 logo 并内嵌，再 spawn。**交付形态是「双击就能开」的单文件 HTML 时，logo/图片必须 base64 内嵌**——相对路径的交付物挪个目录就全员裂图（盲测实锤：`../assets/google.svg` 六个按钮全裂直接输掉评审）；仅多文件+启动说明的项目允许本地路径。**清单里有一个没取到 logo = 🛑 STOP 补齐**（实在取不到才退诚实 placeholder 并明说「X 的 logo 待补」）。各方向 subagent 共用这批 logo。⚠️ 这是对比 / 榜单 / 评测 deck 最常见的翻车点——「只抽了品牌色就开做」就是漏了这道门（2026-06-06 五大 Coding Agent PPT 实测翻车，见 brand-asset-protocol 反例）。
...
- 取到的真图用 base64 内嵌或本地路径，传给各方向 subagent 复用
- ❌ **内容必需的图绝不用 CSS 色块 / SVG 几何糊弄**——鹦鹉网站没有鹦鹉图 = 失败
- **取图失败三级兜底（不许卡死）**：① 公共领域库找不到 → 换 Unsplash/Pexels；② 全网取不到合适真图 → 用户确认有生图能力则走 `huashu-gpt-image` 以参考图为基底生成；③ 仍不行 → 标注「图待补」诚实 placeholder **继续 spawn 多方向 subagent，不卡流程**，交付时一句话告诉用户「这版图是占位，真图待补」。⚠️ **取图失败是「降级继续」，不是 🛑 STOP**——别让取图卡死整个设计。

> 来自花叔实测：鹦鹉案例里「先判断图片必需 → 选对获取策略（Edward Lear 公共领域博物插画）」是出彩的关键。**素材齐了再设计，不是边设计边占位。**

**Phase 4 · 多方向并行 subagent，各生成一版真实视觉（核心）**

> ✅ **这是 Fallback 的 default 动作**：用户**无需主动要求**「用多套逻辑」「帮我找最佳设计师」——只要触发了顾问模式（用户没给明确风格参考），就**自动**并行跑下面的逻辑组合（数量按 Design Mode：FULL 三套全跑 / STANDARD 跑其中两套）。目标是让什么都不懂的普通用户，零额外要求也能拿到顶级设计。

> 🔴 **选择无效铁律**（花叔 2026-06 实测确认）：绝不让用户在「只有文字、没看到视觉」时选风格——用户没依据。所以不抛文字单选题，而是**并行启动多个 subagent、按当前 Design Mode 的数量各跑一套互补逻辑**（FULL 3 套 / STANDARD 2 套），各产出一版真实视觉，一次性摆出来让用户选「看得见的东西」。这些 subagent **独立 context、互不参考**（避免趋同），并行是为了更快 deliver。

> ⚙️ **无法 spawn subagent 时**走串行（降级判定看实际能力，不看模型厂商，见「Runtime 能力降级」）：每套开跑前只读 spec、清空对上一套的记忆、不许参考已生成的版本，并用不同 anchor（轮盘号 / 参照案例 / 设计师名）物理隔离趋同。串行也**必须出足当前 Design Mode 要求的方向数**（FULL 3 版 / STANDARD 2 版），不许偷懒并成一版。prompt 里只喂 spec，别把别套的逻辑一起写进去。
...
并行执行规范（各方向的 subagent 共用）：
- 用**用户真实内容**（非 Lorem），各版同内容只换设计逻辑，方便横向对比
- **各版的布局骨架必须互异**：导航/构图/内容区结构至少一项结构性不同，不许两版共用同一骨架只换色换字体（盲测实锤：共用骨架会被评审一眼识破「换皮」）
...
- 纯 HTML/CSS 单文件；**内容必需的图用 Phase 3.5 取的真图**（各版共用），仅装饰/抽象图才用 CSS 几何/SVG/纯色块，绝不留空占位
- 🎞️ **PPT / deck 场景必走 deck 模板（绝不写竖向平铺长页！）**：每页独立 `<section>`（1920×1080）套 `assets/deck_index.html` 外壳，各版只换视觉风格、deck 骨架统一（架构规则与概览墙细节见「技术红线」+ `references/slide-decks.md`）。截图按**单页** 1920×1080 截；**单页内容绝不自带页码/进度标记**——页码由 deck 外壳统一承载（实测出过「02/03」+「6/16」双页码打架）。**多页deck走方向探索时，各版各出2页代表页**（兼作deck链的showcase），选定方向后再批量其余页
...
- ✅ **产出自检（防偷懒，进 Phase 5 前必查）**：确认 `design-demos/` 下的真实 .html 数量**达到当前 Design Mode 要求**（FULL 3 个 / STANDARD 2 个 / FAST 不走本节）——不足 = 没走完方向探索，补齐再往下；不许在 FULL/STANDARD 下只做一版交差
- 各版全部完成后**一起展示对应截图**，每版标明：用了哪套逻辑、具体哪个风格/参照案例/设计师，一句话说为什么
...
**Phase 5 · 用户基于「看到的真实视觉」选择**（第一次有效选择）：看完各版真实截图，选一版深化 / 混合（"轮盘版的配色 + 设计师版的布局"）/ 微调 / 全部重来 → 重跑方向探索。**用户选定后，立刻把「展示了哪几版、截图路径、用户选择原话」写入项目目录 `direction-approved.md`**（Gate文件协议）。
...
1. **理解需求**：
   ...
   - 🔴 **方向探索按 Design Mode 执行**：产出新视觉设计的任务，先按「设计复杂度路由」定 FAST / STANDARD / FULL——FAST 直接执行；STANDARD 做轻量方向探索（视觉方向未定才出 2 个方向，已有明确 reference/品牌系统可跳过）；FULL 才走「设计方向顾问（Fallback 模式）」完整 Phase 1-5，真实初稿摆给用户、**用户选定后**才回到这里 Step 2。用户给了风格词/品牌名只改变取材方式，不豁免 FULL 这道门。豁免按 Fallback 节「以下情况直接跳过方向探索」判定，并在 `direction-approved.md` 记原话/理由。
...
9. **（默认）导出视频 · 必带 SFX + BGM**：动画 HTML 的**默认交付形态是带音频的 MP4**，不是纯画面。无声版本等于半成品——用户潜意识感知「画在动但没声音响应」，廉价感的根源就在这里。流水线：
   - **新动画项目默认 HyperFrames 后端**：`npm run check`（五门审计，暗色电影风 `--no-contrast`）→ `npx hyperframes render --fps 60` → `scripts/verify-video.sh` 产物硬校验。选型边界与老 demo 适配器配方见 `references/hyperframes-backend.md`；能力受限的 runtime/单文件交付/纯交互演示仍走下面的自研管线
...
| `direction-approved.md` | 方向探索结果+**用户选择原话**记录（含初稿截图路径）。FULL、以及「视觉方向未定的 STANDARD」必须有；FAST、已选定方向的迭代等按 Fallback 节「以下情况直接跳过方向探索」处理，跳过时在本文件记用户原话/迭代来源（跳过≠可以不写文件） | 需要方向探索的任务在实现开工前；**≥45s长片渲染前有hook硬检查**（scripts/design-gate-hook.sh，缺文件block渲染，用户明说跳过用SKIP_DESIGN_GATE=1显式放行） |
...
**「用户说继续」授权的是进入下一步，不是跳过该步内部的gate**。跳过必须用户明说，且把「用户明示跳过」写进对应gate文件。**Runtime 能力降级不豁免 gate 文件**——降级只减少方向数量和 references 加载，三个gate文件本身照写（写文件不耗上下文）；把检查点问答换成assumption清单时，清单就写进对应gate文件里。
...
| 用户拒绝回答问题清单 | 用户说"不要问了，直接做" | 按「设计复杂度路由」重定模式：问题可以不问（自己补assumption）。全局「直接做，不用出方向」→ FAST 直接执行；FULL 级任务（品牌从零/正式发布/宣传片）仍应出方向——仅当用户明说「别出多版/一版就行」才降为单方案，并在`direction-approved.md`记用户原话 |
...
不要根据模型厂商或 runtime 名称直接判断能力强弱。

只有满足以下实际条件时才降级：
- 无法 spawn / delegate subagent
- 剩余 context 明显不足
- 多文件操作已经出现遗漏
- tool 调用能力有限
- 用户明确要求降低 token / 时间成本
...
references 始终 lazy-load：
只读取当前步骤实际需要的文件。
禁止为了“完整理解 skill”一次性加载全部 references。
...
**附加降级动作（能力吃紧时叠加，不替代上面的阶梯）**：
- 无法 spawn subagent 时改**串行**跑多个方向：每套开跑前只读 spec、清空对上一套的记忆、不许参考已生成的版本，并用三个不同 anchor（轮盘号 / 参照案例 / 设计师名）物理隔离趋同；串行也必须出足当前 Design Mode 要求的方向数，不许并成一版
- deck 默认单文件架构（`assets/deck_stage.js`）——多文件 + 概览墙依赖多轮文件操作，中途放弃会留下坏 deck
- 🛑 检查点问答可在产出中改标 assumption 清单——多轮问答成本高，把「问」换成「可审计的假设」（gate 文件本身照写）
